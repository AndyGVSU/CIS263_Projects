...
namespace gv {
        ...
        vector<E> levelOrder() const {
            ...
            while (parentList.size() > 0) {
                for (Node* x : parentList) {
                    Node* temp;
                    if (x->left != nullptr && x->leftReal) {
                        temp = (x->left).get();
                        childList.push_back(temp);
                    }
                    if (x->right != nullptr && x->rightReal) {
                        temp = (x->right).get();
                        childList.push_back(temp);
                    }
    ...
    private:
        ...
        const E findMin(const shared_ptr<Node>& current) const {
            if (isEmpty())
                throw length_error("Tree is empty");
            if (current->left == nullptr || !current->leftReal)
                return current->data;
            return findMin(current->left);
        }

        const E findMax(const shared_ptr<Node>& current) const {
            if (isEmpty())
                throw length_error("Tree is empty");
            if (current->right == nullptr || !current->rightReal)
                return current->data;
            return findMax(current->right);
        }

        bool insert(const E &item, shared_ptr<Node>& current,
                shared_ptr<Node>& holdLeft,
                shared_ptr<Node>& holdRight) noexcept {
            ...
            if (current == nullptr) {
                ...
                if (holdLeft != nullptr) {
                    ...
                    current->left.reset(holdLeft.get());
                    current->leftReal = false;
                }
                if (holdRight != nullptr) {
                    current->right = make_shared<Node>();
                    current->right.reset(holdRight.get());
                    current->rightReal = false;
                }
                return true;
            }
            else if (item > current->data) {
                if (!(current->rightReal)) {
                    current->right.reset();
                    current->rightReal = true;
                }
                return insert(item, current->right, current, holdRight);
            }
            else if (item < current->data) {
                if (!(current->leftReal)) {
                    current->left.reset();
                    current->leftReal = true;
                }
                return insert(item, current->left, holdLeft, current);
            }
            return false;
        }

        bool remove(const E &item, shared_ptr<Node>& current) {
            if (isEmpty()) {
                throw length_error("Tree is empty.");
            } else {
                if (current == nullptr)
                ...
                current->leftReal) {
                    ...
                    if (current->right != nullptr && current->rightReal) {
                        ...
                        current.reset(current->right.get());
                    }
                    else if (current->left != nullptr && current->leftReal) {
                        current->left->right = current->right;
                        current.reset(current->left.get());
                    } else {
        ...
        void printTree(const shared_ptr<Node>& current, ostream &targetStream = cout) const noexcept {
        ...
        bool contains(const E &val, const shared_ptr<Node>& current) const noexcept {
            if (current == nullptr)
                return false;
            if (val < current->data) {
                if (!current->leftReal)
        ...
        int numberOfNodes(const shared_ptr<Node>& current) const noexcept {
            if (current == nullptr)
                return 0;
        ...
        int numberOfLeaves(const shared_ptr<Node>& current) const noexcept {
            if (current == nullptr)
                return 0;
        ...
        int numberOfFullNodes(const shared_ptr<Node>& current) const noexcept {
            if (isEmpty())
                return 0;
            if (current->left != nullptr && current->right != nullptr)
        ...
        vector<E> remove_leaves(vector<E>& prunedLeaves, const shared_ptr<Node>& current) noexcept {
            if (current == nullptr)
                return prunedLeaves;
            if (current->left == nullptr && current->right == nullptr) {
