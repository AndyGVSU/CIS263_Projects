...
namespace gv {
    ...
    private:
        ...
        const E findMin(const weak_ptr<Node>& current) const {
            if (isEmpty())
        ...
        const E findMax(const weak_ptr<Node>& current) const {
            if (isEmpty())
                throw length_error("Tree is empty");
        ...
        bool insert(const E &item, weak_ptr<Node>& current,
                weak_ptr<Node>& holdLeft,
                weak_ptr<Node>& holdRight) noexcept {
        ...
        bool remove(const E &item, weak_ptr<Node>& current,
                    weak_ptr<Node>& holdLeft,
                    weak_ptr<Node>& holdRight) {
            if (isEmpty()) {
                throw length_error("Tree is empty.");
            } else {
                if (current == nullptr)
                ...
                if (item > current->data)
                    return remove(item, current->right, current, holdRight);
                else if (item < current->data)
                    return remove(item, current->left, holdLeft, current);
                else if (current->right != nullptr &&
                current->rightReal &&
                current->left != nullptr &&
                current->leftReal) {
                    current->data = findMin(current->right);
                    return remove(current->data, current->right, current, holdRight);
                }
                else {
                    if (current->right != nullptr && current->rightReal) {
